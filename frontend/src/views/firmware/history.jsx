...
const FirmwareHistory = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchFirmwareHistory({
        page,
        size: SIZE,
      ...
      else toast.error(`Erro ao carregar histórico: ${err.response?.data?.error || err.message || err}`);
    } finally {
      setLoading(false);
    }
                ...
                {summary.lastUpdate ? (
                  <div className="truncate text-sm">
                    ...
                    <span className="text-muted">
                      {fmtLocal(new Date(summary.lastUpdate.date))}
                    </span>
                  </div>
                ) : (
                  ...
                  {rows.map((r, i) => (
                    <tr key={i} className="border-b border-border/60 hover:bg-surface-muted/40">
                      ...
                      <td className="px-4 py-2.5 text-muted">
                        {r.updatedAt ? fmtLocal(r.updatedAt) : '—'}
                      </td>
