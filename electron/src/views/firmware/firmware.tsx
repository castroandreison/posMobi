...
const lineTime = (l: string): Date | null => {
  let m = l.match(
...
interface FwBlock {
  ...
  [key: string]: string | { url: string; version: string };
}

interface FwType {
  key: 'gateway' | 'mcu' | 'completa';
  label: string;
}

const emptyBlock = (): FwBlock => ({
  id: '',
  name: '',
  gateway: { url: '', version: '' },
  mcu: { url: '', version: '' },
...
interface LegacyUrls {
  [model: string]: {
    [key: string]: { url: string; version: string };
  };
}

function loadLegacyUrls(): LegacyUrls | null {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') return null;
    return parsed as LegacyUrls;
  } catch {
    return null;
  }
}
...
const FirmwareView: React.FC = () => {
  ...
  const [draftBlocks, setDraftBlocks] = useState<FwBlock[]>([]);
  const [draftLinks, setDraftLinks] = useState<Record<string, string>>({});
  const [fwLoading, setFwLoading] = useState(false);
  const [monitor, setMonitor] = useState<Record<string, MonitorEntry>>({});
  const [monitoring, setMonitoring] = useState(false);
  const selectAllRef = useRef<HTMLInputElement>(null);
  const monitorStartedAtRef = useRef<number | null>(null);
  const [blocks, setBlocks] = useState<FwBlock[]>([]);
        ...
        if (d.blocks.length) {
          ...
          if (legacy) {
            const imported = Object.keys(legacy).map((model) => {
              ...
              b.name = model;
              FW_TYPES.forEach((t: FwType) => {
                const src = legacy[model]?.[t.key];
                if (src && typeof src === 'object') {
                  (b[t.key] as { url: string; version: string }) = {
                    url: src.url || '',
                    version: src.version || '',
                  };
                }
            ...
            saveFirmwareBlocks(imported, links)
              .then(() =>
                toast.success('URLs de firmware importadas do armazenamento local')
              )
              .catch(() =>
                toast.error('Não foi possível salvar os dados importados')
              );
          }
        }
      })
  ...
  const loadStations = async (tenantPk?: number | string | null) => {
    setLoading(true);
    try {
      ...
      else toast.error(`Erro ao carregar: ${error.message || err}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAccountChange = (acct: Tenant | null) => {
    setTenant(acct);
    setFilters({
      chargeBoxId: '',
      description: '',
  ...
  const statusOptions = useMemo(
    () =>
      [...new Set(stations.map((s) => (stationStatus(s) as StatusReturn).label))].sort(),
    [stations]
  );
  const onlineOptions = useMemo(
    () =>
      [
        ...new Set(
          stations.map(
            (s) => (onlineStatus(s.lastHeartbeatTimestamp) as StatusReturn).label
          )
        ),
      ].sort(),
    [stations]
  ...
  const filtered = useMemo(() => {
    ...
    return stations.filter((s) => {
      const st = stationStatus(s) as StatusReturn;
      const onl = onlineStatus(s.lastHeartbeatTimestamp) as StatusReturn;
      ...
      if (
        filters.firmware &&
        !q(s.fwVersion || '').includes(q(filters.firmware))
      )
        return false;
  ...
  const idOf = (s: Station): string | number => s.chargeBoxPk || s.chargeBoxId;
  const allSelected =
  ...
  const blockForModel = (model: string | undefined): FwBlock | null => {
  ...
  const urlForType = (s: Station, typeKey: string): string => {
    const block = blockForModel(s.chargePointModel);
    return (
      (block?.[typeKey] as { url: string; version: string } | undefined)?.url?.trim() || ''
    );
  };

  const sendType = async (type: FwType) => {
    const target = filtered.filter((s) => selected.has(idOf(s)));
    if (!target.length) {
      toast.warning(
        'Selecione ao menos um carregador na tabela antes de enviar'
      );
      return;
    }
    ...
    if (!groups.size) {
      const modelsList = [...new Set(target.map((s) => s.chargePointModel || '—'))];
      console.warn('[FIRMWARE] Modelos sem link', {
        selecionados: modelsList,
        configurados: Object.keys(modelLinks),
      });
      toast.error(
        `Nenhum carregador tem link de ${type.label}. Modelos selecionados: ${modelsList.join(', ')}`
      );
      return;
    }
    ...
    try {
      ...
      for (const [url, group] of groups) {
        const data = (await updateFirmware(group, url)) as Record<string, unknown>;
        console.log('[FIRMWARE] Resposta updateFirmware', { url, data });
        if (data?.statusCode) status = data.statusCode as string;
        const tid = (data?.data as Record<string, unknown>)?.taskId ?? data?.taskId;
        if (tid) taskIds.push(String(tid));
      }
      ...
      console.error('[FIRMWARE] Erro ao enviar', error?.response || err);
      if (isSessionExpired(err)) toast.error('Sessão expirada');
      else
        toast.error(
          `Falha no upload ${type.label}: ${error.response?.data?.error || error.message || err}`
        );
    } finally {
      setSending('');
    }
  ...
  const startMonitor = (targets: Station[]) => {
  ...
  const refreshMonitor = async () => {
    ...
    try {
      const cloudText = await fetchLogs(
        'cloud',
        tenant?.alias || 'Intelbras'
      ).catch(() => '');
      ...
      setMonitor((prev) => {
        ...
        Object.keys(next).forEach((stationId) => {
          ...
          const cur = next[stationId] || ({} as MonitorEntry);
          let status = cur.status;
          if (parsed.status) status = parsed.status;
          else if (parsed.rebooted && !cur.rebooted) status = 'Reiniciando...';
          const patch: MonitorEntry = { ...cur, status };
          const trail: string[] = [];
          ...
          if (parsed.firmwareVersion)
            patch.firmwareVersion = parsed.firmwareVersion;
          if (parsed.gatewayVersion)
            patch.gatewayVersion = parsed.gatewayVersion;
      ...
      const error = err as { message?: string };
      if (isSessionExpired(err)) toast.error('Sessão expirada');
      else toast.error(`Erro no monitoramento: ${error.message || err}`);
    }
  };
    ...
    fetchChargepoints()
      .then((list) =>
        setAllModels(
          [
            ...new Set(
              (list as Station[]).map((s) => s.chargePointModel || 'Sem modelo')
            ),
          ].sort()
        )
      )
  ...
  const updateDraftBlock = (
    ...
    setDraftBlocks((prev) =>
      prev.map((b) => {
        ...
        return {
          ...b,
          [typeKey]: { ...(b[typeKey] as { url: string; version: string }), [field]: value },
        };
      })
    );
  };

  const updateDraftName = (blockId: string, value: string) => {
    setDraftBlocks((prev) =>
  ...
  const setDraftLink = (model: string, blockId: string) => {
  ...
  const saveUrls = async () => {
    ...
    try {
      const validBlocks = draftBlocks.filter(
        (b) =>
          b.name.trim() ||
          FW_TYPES.some((t) => (b[t.key] as { url: string; version: string })?.url?.trim())
      );
      const result = (await saveFirmwareBlocks(validBlocks, draftLinks)) as {
        blocks?: FwBlock[];
      ...
      const error = err as { message?: string };
      toast.error(`Falha ao salvar: ${error.message || err}`);
    } finally {
      setFwLoading(false);
    }
  };
        ...
        <CardBody className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters((f) => ({ ...f, online: e.target.value }))
              }
            >
              <option value="">Todos</option>
              {onlineOptions.map((o) => (
                  ...
                  {filtered.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5">
                          <Badge
                            tone={(stationStatus(s) as StatusReturn).tone as 'success' | 'danger' | 'warning' | 'primary' | 'muted'}
                          >
                            {(stationStatus(s) as StatusReturn).label}
                          </Badge>
                        </td>
                        <td className="px-4 py-2.5">
                          {mon ? (
                            <>
                              <Badge
                                tone={fwStatusTone(mon.status) as 'success' | 'danger' | 'warning' | 'primary' | 'muted'}
                              >
        ...
        <CardBody className="flex flex-wrap gap-3">
          {FW_TYPES.map((t: FwType) => (
            ...
            >
              <UploadCloud
                size={16}
                className={sending === t.key ? 'animate-pulse' : ''}
              />
              {sending === t.key
                ? `ENVIANDO ${t.label.toUpperCase()}...`
                : t.label}
            </Button>
          ))}
          {selected.size === 0 && (
            ...
            <div className="flex-1 space-y-6 overflow-y-auto p-6">
              <div className="space-y-3">
                ...
                {draftBlocks.length === 0 && (
                  <p className="text-sm text-muted">
                    Nenhum carregador cadastrado. Clique em "Novo carregador" para
                    criar um.
                  </p>
                )}
                {draftBlocks.length > 0 && (
                          ...
                          <tr key={b.id} className="border-b border-border/60">
                            <td className="px-3 py-2">
                              <input
                                ...
                                value={b.name}
                                onChange={(e) =>
                                  updateDraftName(b.id, e.target.value)
                                }
                              />
                            </td>
                            {FW_TYPES.map((t: FwType) => (
                              <td key={t.key} className="px-3 py-2">
                                <div className="flex flex-col gap-1.5">
                                  <input
                                    ...
                                    placeholder="URL"
                                    value={
                                      (b[t.key] as { url: string; version: string })?.url ||
                                      ''
                                    }
                                    onChange={(e) =>
                                      updateDraftBlock(
              ...
              <div className="space-y-3 border-t border-border pt-4">
                ...
                <p className="text-xs text-muted">
                  Cada modelo só pode ser vinculado a um carregador. Todas as
                  estações do modelo usarão o carregador selecionado.
                </p>
