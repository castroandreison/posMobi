...
import { fetchLogs, downloadLogs, fetchChargepoints, isSessionExpired } from '../../api/client';
import { Card, CardHeader, CardBody } from '../../components/ui/card';
import { Button } from '../../components/ui/button';
import AccountSelect from '../../components/ui/accountselect';

const levelBadge = (line: string): { label: string; color: string } | null => {
...
const directionTag = (line: string): { label: string; color: string } | null => {
...
const LogView: React.FC = () => {
  ...
  const [tenant, setTenant] = useState<Account | null>(null);
  ...
  const loadLogs = async (src: string, tn: string) => {
    setLoading(true);
    setError('');
    try {
  ...
  const loadStations = async (tenantPk?: number | null) => {
    try {
      setStations((await fetchChargepoints(tenantPk)) as Station[]);
    } catch (e) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const blob = (await downloadLogs(tenantAlias)) as unknown as Blob;
  ...
  const handleAccountChange = (acct: Account | null) => {
    setTenant(acct);
