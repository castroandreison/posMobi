...
import { fetchChargepoints, fetchLogs, fetchTenants, parseStationFault, isSessionExpired, stationStatus, onlineStatus, formatToSaoPaulo } from '../../api/client';
import { resolveDefaultTenant, setStoredTenantPk } from '../../api/defaultTenant';
import { Card, CardHeader, CardBody } from '../../components/ui/card';
import { Badge } from '../../components/ui/badge';
import { Button } from '../../components/ui/button';
import FirmwareUpdate from '../../components/ui/firmwareupdate';
...
const KPIS: Array<{
  key: 'total' | 'active' | 'occupied' | 'faulted';
  label: string;
...
const Dashboard = () => {
  const [stations, setStations] = useState<Station[]>(DASH_CACHE.stations || []);
  const [loading, setLoading] = useState(false);
  const [lastUpdate, setLastUpdate] = useState<Date | null>(DASH_CACHE.lastUpdate);
  ...
  const [logModal, setLogModal] = useState<Station | null>(null);
  const [logLines, setLogLines] = useState<string[]>([]);
  const [logLoading, setLogLoading] = useState(false);
  const [logError, setLogError] = useState('');
  const [faults, setFaults] = useState<Record<string, string>>(DASH_CACHE.faults);
  const [initialized, setInitialized] = useState(false);

  const loadStations = async (tenantPk?: number | string | null) => {
    setLoading(true);
    try {
      const list = await fetchChargepoints(tenantPk) as Station[];
      setStations(list);
  ...
  const openStationLog = async (station: Station) => {
    setLogModal(station);
    setLogLines([]);
    setLogError('');
    setLogLoading(true);
    try {
      ...
      const relevant = (text || '')
        ...
        .filter((l: string) => l.includes(station.chargeBoxId || ''))
        .slice(-200);
      ...
      setLogError(
        String((response?.data as Record<string, unknown>)?.error || error.message || err)
      );
    } finally {
      setLogLoading(false);
    }
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {st.label === 'Falha' && (
                            <>
                              ...
                              {s.chargeBoxId && faults[s.chargeBoxId] && (
